...
export class SpaceService implements ISpaceRepository {
    ...
    async get(input: SpaceInput): Promise<SpaceEntity | null> {
        const result: SpaceEntity | null = await this.spaceRepo.get(input).catch((error) => {
    ...
    async add(input: SpaceInput): Promise<SpaceEntity | null> {
        const result: SpaceEntity | null = await this.spaceRepo.add(input).catch((error) => {
            throw new Error(error);
        });
        return result;
    }
    async update(input: SpaceInput): Promise<SpaceEntity | null> {
        const result: SpaceEntity | null = await this.spaceRepo.update(input).catch((error) => {
            throw new Error(error);
        });
        return result;
    }
